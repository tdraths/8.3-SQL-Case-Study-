...
select distinct mems.firstname || ' ' || mems.surname as member, fac.name as facility
    from
        Members mems
        inner join Bookings bkg
            on bkg.memid = mems.memid
        inner join Facilities fac
            on fac.facid = bkg.facid
where bkg.facid in (0,1)
order by member
...
select 	mems.firstname as memfirstname, 
...
select facid, memid, sum(slots) as "Total Slots"
    from Bookings
